...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        List<StateInfo> firstFiveStates = states.stream().limit(5).toList();
        ...
        List<StateInfo> lastFiveStates = states.stream().skip(45).toList();
        ...
        StateInfo cardinalState = states.stream().filter(s->s.getStateBird().equals("cardinal")).toList().get(0);
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream().anyMatch(s->s.getLowestElevationInFeet()<0);
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(s->s.getStateAnthem() != null);
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(s->s.getStateMotto().split(" ").length == 1);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream().collect(averagingDouble(s->s.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().collect(summingInt(s->s.getCapital().getAverageYearlyPrecipitationInInches()));
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream().collect(Collectors.groupingBy(s->s.getTimeZones().toString(), Collectors.counting()));
